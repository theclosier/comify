...
import { ChevronLeft, Save, MapPin, Users, Globe, Share2, Upload, FileText } from "lucide-react";
...
export default function ClubEditPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = use(params);
    const club = MOCK_COMMUNITIES.find(c => c.id === id);

    // Form State
    const [formData, setFormData] = useState({
        name: club?.name || '',
        description: club?.description || '',
        location: 'İstanbul, Türkiye', // Mock default
        website: 'https://ornek.com', // Mock default
        category: 'Teknoloji', // Mock default
    });
