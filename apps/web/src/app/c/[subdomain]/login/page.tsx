...
export default function CommunityLoginPage({ params }: { params: Promise<{ subdomain: string }> }) {
            ...
            <div className="w-full max-w-md space-y-8 animate-in slide-up duration-700">
                ...
                <div className="text-center space-y-4">
                    <p className="text-sm text-stone-500">
                        Hesabınız yok mu? <Link href={`/c/${subdomain}/join`} className="font-bold text-stone-900 hover:text-teal-600 transition-colors">Aramıza Katılın</Link>
                    </p>
                    <Link href={`/c/${subdomain}`} className="block text-xs font-bold text-stone-400 hover:text-stone-600 transition-colors">
                        Topluluğa Dön
                    </Link>
                </div>
            </div>
